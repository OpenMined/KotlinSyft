package org.openmined.syft.unit.monitor.network.client
...
internal class DelayedHttpClientMock(
        ...
        mockedClient.stub {
            ...
            on { downloadSpeedTest(any(), any()) }.thenReturn(
                if (downloadSpeedSuccess)
                    Single.just(Response.success(ByteArray(1 * MB).toResponseBody())).delay(10, TimeUnit.SECONDS)
                else
                    Single.just(
                        Response.error(
                            403,
