...
class NetworkStatusRepository(private val configuration: SyftConfiguration)  {
    ...
    fun updateNetworkValidity(
        ...
        networkManager.getNetworkCapabilities(networkManager.activeNetwork)?.let { capabilities ->
            networkStateModel.networkValidity = constraints.all { capabilities.hasCapability(it) }
        } ?: throw Exception("Unknown network. Cannot detect network properties")
    }

    fun updatePing(workerId: String, networkStateModel: NetworkStateModel): Completable {
    ...
    fun updateDownloadSpeed(workerId: String, networkStateModel: NetworkStateModel) =
